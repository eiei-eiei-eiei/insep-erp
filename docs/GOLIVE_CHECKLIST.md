...
## หลังอัปเดต D90 + D91 (ยกเลิกออเดอร์ไม่โผล่บนฟอร์ม ภส. + ปิดเดือนสรรพสามิต · 2026-09-01)

### ① ลง migration 0057 + 0058 + 0059 ทุก DB — **ก่อน** git push

```bash
npm run db:push:all -- --apply --backup="D:/insep-erp-backup/2026-09-01-monthclose"
```

เป็นแบบ *เพิ่มของ* (คอลัมน์ `log_product.ref_no` + `excise_hidden` · ตาราง `excise_month_close`)
🚨 **ไม่มีการแก้ข้อมูลเดิมย้อนหลังเลย** — ฟอร์มของเดือนที่ผ่านมาที่อาจยื่นไปแล้วจะไม่ขยับ

### ② 🔴 งานประจำเดือนที่เพิ่มขึ้น — ยื่นงบเดือนแล้วให้กด "ปิดเดือน"

**ผลิต → รายงานสรรพสามิต → การ์ด "ปิดบัญชีสรรพสามิตของเดือนนี้" → ปุ่ม ปิดเดือน**

| | |
|---|---|
| **ทำเมื่อไร** | หลังยื่น **ภส.๐๗-๐๔ งบเดือน** ให้กรมสรรพสามิตเรียบร้อยแล้ว (ยื่นได้ถึงวันที่ 15 ของเดือนถัดไป) |
| **ทำไมต้องกด** | หลังปิดแล้ว การยกเลิกบิลจะ **ไม่เปลี่ยนตัวเลขบนฟอร์มของเดือนนั้น** — ฟอร์มที่ยื่นไปแล้วต้องนิ่ง |
| **ไม่กดจะเป็นอะไร** | ยกเลิกบิลย้อนหลังแล้วตัวเลขเดือนนั้นจะขยับ ทั้งที่ยื่นไปแล้ว |
| **ใครกดได้** | เจ้าของกิจการ (บทบาทอื่นเห็นสถานะได้อย่างเดียว) |
| **กดผิดทำยังไง** | ปุ่ม **ถอนปิดเดือน** — ระบบคำนวณการซ่อนใหม่ให้ทันทีและบอกว่ากระทบกี่คู่ |

★ **การกดสร้าง PDF ไม่ล็อกอะไรทั้งนั้น** — พิมพ์บัญชีประจำวันให้เจ้าหน้าที่ตรวจได้ตลอดตามปกติ

### ③ ของเก่าที่ค้างอยู่ — ตรวจครั้งเดียวตอนขึ้นระบบ

ถ้าเคยยกเลิกบิลไปแล้วในเดือนที่ **ยังไม่ได้ยื่นงบเดือน** ให้เปิดเดือนนั้นแล้วดูการ์ด —
ถ้าขึ้นแถบเหลือง *"มีคู่ จ่าย/รับ ของบิลที่ยกเลิก N คู่ที่ยังแสดงบนฟอร์ม"* ให้กด
**คำนวณใหม่ตามจริง** เพื่อเอาออก

🚨 **อย่ากดกับเดือนที่ยื่นงบเดือนไปแล้ว** — ตัวเลขบนฟอร์มจะไม่ตรงกับที่ยื่น
(ระบบไม่ทำให้เองโดยตั้งใจ เพราะไม่มีทางรู้ว่าเดือนไหนยื่นไปแล้วบ้าง)

### ④ สิ่งที่ควรรู้

- **ยกเลิกบิลของเดือนที่ปิดแล้ว** → หน้าขายจะขึ้นแถบ**เหลือง** บอกว่าคู่ จ่าย/รับ ยังอยู่บนฟอร์ม
  พร้อมบอกให้มาถอนปิดเดือนถ้ายังไม่ได้ยื่นจริง — **บิลถูกยกเลิกเรียบร้อยแล้ว** แถบเหลืองไม่ได้แปลว่าล้มเหลว
- **สต็อกไม่เกี่ยวกับการซ่อน** — หน้าสต็อก/ประวัติในแอปเห็นทุกแถวตามจริงเสมอ
  ซ่อนเฉพาะบนกระดาษที่ยื่นราชการ
- **ปิดเดือนไม่ได้ล็อกการแก้ข้อมูล** — ยังลงบันทึกย้อนหลังได้ แต่การ์ดจะฟ้องว่า
  *"ข้อมูลปัจจุบันต่างจากตอนปิดเดือน"* ให้เห็นว่าต้องยื่นเพิ่มเติมหรือไม่

---

## หลังอัปเดต D92 (เตือนกำหนดยื่นงบเดือนสรรพสามิตเข้า LINE · 2026-09-01)

### ① ไม่ต้องลง migration และไม่ต้องแก้ Vercel

ใช้ตารางเดิม · route เดิม (`/api/cron/tax-reminder`) · cron entry เดิม · `CRON_SECRET` ตัวเดิม
→ **push อย่างเดียวจบ** (ต้องเคยตั้ง `CRON_SECRET` ไว้แล้วตาม D88 ข้อ ③)

### ② 🔴 ต้องกรอก "เลขสรรพสามิต" ของกิจการ ไม่งั้นไม่ได้รับการเตือน

**ตั้งค่า → กิจการ → เลขสรรพสามิต**

ระบบใช้ช่องนี้เป็นตัวบอกว่า *กิจการนี้เป็นโรงสุราที่ต้องยื่นงบเดือน* — ไม่กรอก = ไม่เตือน

> 🚨 ตอนตรวจ 2026-09-01 พบว่า **ลูกค้าที่ซื้อโมดูลผลิตยังไม่ได้กรอกช่องนี้**
> ให้ไล่กรอกให้ครบก่อน ไม่งั้นเขาจะรอการเตือนที่ไม่มีวันมา
> · ในแอปมีบรรทัดเหลืองเตือนไว้ที่ ผลิต → รายงานสรรพสามิต แล้ว

### ③ สิ่งที่จะเกิดขึ้นทุกเดือน

| วันที่ | เกิดอะไร |
|---|---|
| **12 ของทุกเดือน** | ถ้ายังไม่ได้กด **ปิดเดือน** ของงวดก่อนหน้า → ยิงเข้ากลุ่ม LINE *"งบเดือน (ภส.๐๗-๐๔) งวด … — ยื่นภายใน 15 …"* |
| หลังยื่นจริง | กด **ปิดเดือน** ในแอป → เดือนถัดไปไม่เตือนงวดนี้อีก |

- **ครั้งเดียวต่องวดต่อกิจการ** — ไม่ตามจิก
- 🚨 **ข้อความไม่มีตัวเลขเงินใด ๆ** (กลุ่ม LINE มีคนที่ไม่ควรเห็นตัวเลขของกิจการ)
- 🚨 **ระบบไม่เลื่อนวันหยุดราชการให้** — วันที่ 15 คือวันที่ 15 ตามปฏิทิน
- วันเดียวกันจะมีข้อความของ **ภพ.30** มาด้วย (ครบกำหนดวันที่ 15 เหมือนกัน) —
  เป็น **2 ข้อความแยกกันโดยตั้งใจ** เพราะคนละกรมและคนละสิ่งที่ต้องไปกด

### ④ ตรวจก่อนใช้จริงโดยไม่ต้องส่งเข้ากลุ่ม

```bash
curl -s -H "Authorization: Bearer <CRON_SECRET>" "https://<โดเมน>/api/cron/tax-reminder?dry=1&date=2026-09-12"
```

`dry=1` = คำนวณอย่างเดียว **ไม่ส่ง LINE** — ดูได้ว่าใครจะได้ข้อความอะไรบ้าง

---

## กลั่นหลายรอบ (แช่สมุนไพรแล้วกลั่นซ้ำ) — D94

> ข้ามได้ถ้าโรงคุณกลั่นรอบเดียวจบ · แท็บ **กลั่นซ้ำ** จะโชว์อยู่เสมอสำหรับเส้นทางสุรากลั่น
> (ระบบไม่มีทางรู้ว่าโรงไหนกลั่นซ้ำ — การซ่อนก็คือการเดา)

### ① ต้องทำก่อนใช้

| สิ่งที่ต้องทำ | ที่ไหน | ถ้าไม่ทำจะเป็นยังไง |
|---|---|---|
| ลง migration **0061** ทุก DB | `npm run db:push:all -- --apply` | แท็บกลั่นซ้ำเปิดไม่ได้ |
| **ขึ้นทะเบียนสมุนไพรเป็นวัตถุดิบ** | ผลิต → จัดการข้อมูล → วัตถุดิบ | ตัดเข้าบัญชี ภส.๐๗-๐๑/๑ ไม่ได้ · ในฟอร์มรอบจะขึ้นเตือนสีเหลืองว่าให้ไปเพิ่มก่อน |
| ตั้ง **ประเภทสุรา = สุรากลั่น** ให้สินค้าที่จะกลั่นซ้ำ | ผลิต → จัดการข้อมูล → สินค้า | สินค้านั้นไม่โผล่ในดร็อปดาวน์ของแท็บกลั่นซ้ำ |

### ② กติกาที่ต้องเข้าใจก่อนกด

- **ยอดที่กรอกตอนเปิดล็อต = ยอดที่ยกออกจากถัง** (เช่น 240 ล.) ไม่ใช่ยอดที่ได้หลังกลั่นซ้ำ (180 ล.)
  🚨 กรอกยอดหลังกลั่นซ้ำเมื่อไหร่ **คงเหลือสุรากลั่นบนฟอร์มจะพองสะสมตลอดกาล** โดยไม่มีอะไรฟ้อง
- ระบบเขียนแถวปรุงลงฟอร์ม **2 ท่อน คนละวัน**: *ยกไปปรุง* (วันเริ่มแช่) และ *ปรุงเสร็จ* (วันปรับดีกรี)
  → แก้/ลบได้ที่แท็บ **กลั่นซ้ำ** ทางเดียว · ที่แท็บปรุงจะเป็นปุ่มเทา
- สมุนไพรถูกตัดออกจากบัญชีวัตถุดิบ **วันเดียวกับวันเริ่มแช่** พร้อมเลขอ้างอิง `S1/69 รอบ 2`
  ⇒ เจ้าหน้าที่เปิด ๐๗-๐๑/๑ กับ ๐๗-๐๒/๑(๑) มาเทียบแล้วเห็นภาพจบในตัว

### ③ ⚠️ ยังต้องยืนยันกับสรรพสามิตด้วยตัวเอง

ปรินต์ฟอร์มเดือนที่มีล็อตกลั่นซ้ำคร่อมเดือนไปถาม 3 ข้อ:

1. บรรทัด *"นำไปปรุง 240"* ที่ไม่มีผลผลิตในเดือนนั้น — รับได้ไหม หรือต้องเขียนแบบอื่น
2. แอลกอฮอล์บริสุทธิ์หายไป ~30% ในขั้นปรุง (เพราะกลืนการกลั่นซ้ำเข้าไป) — ต้องมีเอกสารประกอบอะไรเพิ่ม
3. มีแบบฟอร์มเฉพาะสำหรับโรงที่กลั่นซ้ำหรือเปล่า

ถ้าคำตอบไม่ตรงกับที่ทำไว้ **แก้ได้ที่ `dilutePlan()` จุดเดียว** ไม่ต้องรื้อ

---

## D95 — "ยื่นแล้ว" แยกจาก "กดพิมพ์แล้ว" + วิธียื่นแบบภาษี (migration 0071)

### ① ต้องทำก่อนใช้

| สิ่งที่ต้องทำ | ที่ไหน | ถ้าไม่ทำจะเป็นยังไง |
|---|---|---|
| ลง migration **0071** ทุก DB | `npm run db:push:all -- --apply` | แท็บเอกสารสรรพากรเปิดไม่ได้ (ตาราง `tax_filings` ยังไม่มี) |
| ตั้ง **วิธียื่นแบบภาษี** ของแต่ละกิจการ | ตั้งค่า → กิจการ | ใช้งานได้ปกติ แต่ระบบจะเตือนตามกำหนด **กระดาษ** (เร็วกว่า) และบอกในข้อความว่ายังไม่ได้ตั้ง |

### ② กติกาที่ต้องเข้าใจก่อนกด

- **"ยื่นแล้ว" ≠ "จ่ายแล้ว"** — ปุ่มยื่นไม่แตะเงินหรือบิลใด ๆ เป็นการบอกระบบว่าส่งแบบแล้ว
  เพื่อ**หยุดการเตือนเข้ากลุ่ม LINE** ของงวดนั้น
- **เดือนที่ทุกอย่างเป็นศูนย์ยังต้องยื่น ภพ.30** → ปุ่ม **ยื่นแล้ว** กดได้แม้ปุ่มจ่ายจะเทา
- **บันทึกจ่ายสำเร็จ = ระบบติ๊ก "ยื่นแล้ว" ให้เอง** (จ่ายได้แปลว่ายื่นแล้ว) แต่**ไม่กลับกัน**
- **ถอนการบันทึกจ่าย ไม่ถอนการยื่น** — ถอนเพราะกรอกยอดผิด ไม่ได้แปลว่าไม่ได้ยื่น
  ถ้าจะถอนการยื่นด้วย ต้องกดปุ่ม **ถอนการบันทึกยื่น** อีกที (เฉพาะหัวหน้าบัญชี/เจ้าของ)
- **การกด "สร้าง ภพ.30 / สร้าง ภงด.3/53" ไม่ปิดการเตือนอีกต่อไป** — กดดูตัวเลขกลางเดือนได้
  โดยไม่ทำให้การเตือนหายไป (นี่คือสิ่งที่งานนี้แก้)

### ③ จะได้ข้อความเตือนวันไหนบ้าง (ต่อแบบ ต่องวด)

| จังหวะ | ยื่นกระดาษ (ภพ.30) | ยื่นออนไลน์ (ภพ.30) | งบเดือน ภส.๐๗-๐๔ |
|---|---|---|---|
| ล่วงหน้า 3 วัน | 12 | 20 | 12 |
| วันสุดท้าย | 15 | 23 | 15 |
| เลยกำหนด (ครั้งเดียว) | 16 | 24 | 16 |

ภงด.3/53 เร็วกว่า ภพ.30 8 วัน (กระดาษครบกำหนดวันที่ 7 · ออนไลน์วันที่ 15)
🚨 **ไม่เลื่อนวันหยุดราชการให้** (ไม่มีปฏิทิน — เตือนเร็วไปดีกว่าเดาแล้วช้า)

### ④ ครั้งแรกหลังติดตั้ง

งวดเก่าที่ยื่นไปแล้วแต่ **ไม่เคยบันทึกจ่าย** (เช่นเดือนที่ยอดเป็นศูนย์) จะถูกเตือนอีกครั้ง
จนกว่าจะกดปุ่ม **ยื่นแล้ว** — ไม่ใช่บั๊ก · ระบบไม่มีบันทึกว่าเดือนไหนยื่นจริงมาก่อน
จึงเติมย้อนหลังให้เฉพาะงวดที่มีหลักฐานการจ่ายเท่านั้น

---

## โมดูลบาร์/POS — D96

### ① ต้องลง migration ก่อนใช้

```bash
npm run db:push:all -- --apply
```

`0063`–`0069` (บาร์ทั้งชุด) — **ลงให้ครบทุก DB** ก่อน push ขึ้น production

### ② ค่าที่ต้องกรอกเอง — ไม่มีค่าปริยาย ระบบไม่เดาให้

| ตั้งที่ | ค่า | ถ้าไม่ตั้งจะเป็นยังไง |
|---|---|---|
| บาร์ → ตั้งค่าบาร์ | **กิจการของบาร์** | ทำอะไรไม่ได้เลย · 🚨 ระบบ**จงใจไม่เดา**เป็นกิจการหลัก เพราะบาร์มักเป็นรายได้ของอีกกิจการหนึ่ง เดาผิดแล้วยอดทั้งคืนไปลงบัญชีโรงกลั่น |
| บาร์ → ตั้งค่าบาร์ | **เลขพร้อมเพย์** | ปุ่ม QR ปิดตาย |
| บาร์ → ตั้งค่าบาร์ | **รอบขาย** (เช่น 18:00–03:00) | บิลที่ข้ามเที่ยงคืนจะไปนับเป็นยอดของวันถัดไป |
| บาร์ → ตั้งค่าบาร์ | **บัญชีที่รายได้บาร์เข้า** | 🔴 **กดลงบัญชีไม่ได้เลย** |
| บัญชี → ตั้งค่า | **ติ๊กบัญชีนั้นว่านับเข้าระบบภาษี** | 🔴 ลงบัญชีสำเร็จ แต่**ยอดบาร์ไม่ขึ้นใน ภพ.30** (หน้าตั้งค่าบาร์จะเตือนให้ถ้าลืม) |

### ③ 🚨 สแกน QR ด้วยแอปธนาคารจริงหนึ่งครั้งก่อนเปิดร้าน

ตั้งค่าบาร์ → **ทดสอบ — สร้าง QR 1 บาท** → สแกน → **อ่านชื่อบัญชีปลายทางด้วยตา**

เลขพร้อมเพย์ผิด **QR ยังสแกนติดปกติ** แต่เงินเข้าบัญชีคนแปลกหน้าโดยไม่มีอะไรในระบบฟ้อง
— ตาคนคือด่านเดียวที่ตรวจได้

### ④ เครื่องพิมพ์

Xprinter XP-T80Q ต่อบลูทูธ + แอป **RawBT** บน Android
→ กดพิมพ์ในแอปแล้วเลือก RawBT ในกล่องพิมพ์ · **พิมพ์จริงหนึ่งใบก่อนเปิดร้าน**
(ความกว้าง 72 มม. และความคมของ QR บนกระดาษความร้อน เบราว์เซอร์มองไม่เห็น)

### ⑤ ⚠️ ถ้ากิจการบาร์จด VAT — ต้องไปขออนุมัติกับสรรพากรเอง

ระบบออก **ใบกำกับภาษีอย่างย่อ** ให้ทุกใบตั้งแต่ปิดบิล และถอด VAT ลงบัญชีให้ถูกต้องแล้ว
**แต่การใช้เครื่อง POS ออกใบกำกับภาษีอย่างย่อต้องยื่นขออนุมัติกรมสรรพากรก่อน**

⚠️ ผมไม่ใช่ที่ปรึกษาภาษี — ให้ผู้ทำบัญชียืนยันขั้นตอนนี้

★ ลูกค้าที่ขอ **ใบกำกับภาษีเต็มรูป** (มีชื่อ/ที่อยู่/เลขภาษีผู้ซื้อ) **ยังทำไม่ได้** ในรอบนี้
