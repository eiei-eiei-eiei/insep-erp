...
`lib/shared/guard.ts` (`requireCap()` ที่หัวทุก page) · `roles.test.ts` +28 · `rolesSql.test.ts` +16 ·
`workspaces.ts`/`tabs.ts` เปลี่ยนจาก `roles[]` เป็น `cap` · migration **0051** (~880 บรรทัด) ·
`scripts/gen-0051.mjs` · `tests/tenant/role-caps.test.ts` +46 · `harness.seedUser()` ·
build/lint/test **713** + `test:tenant` **184** ผ่าน · ชุดเทส `docs/TESTING.md` ส่วนที่ 43
