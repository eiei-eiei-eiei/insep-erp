...
/**
 * ตัวดำเนินการของ 1 ขั้น — **ชุดปิด 4 ตัว ห้ามเพิ่ม**
 *
 * 🚨 ที่เพิ่ม +−× เข้ามาจากเดิมที่มีแต่ ÷ (D70) **ไม่ได้แปลว่ากติกา "ห้ามทำภาษาสูตร" ถูกยกเลิก**
 *    สิ่งที่กติกานั้นปกป้องคือ *ไม่มี parser · ไม่มีลำดับความสำคัญของตัวดำเนินการ ·
 *    เส้นทางคำนวณนับได้จนครบ* — การขยายชุดปิดยังรักษาครบทั้ง 3 ข้อ
 *    ถ้าวันหนึ่งมีคนอยากได้ "วงเล็บ" หรือ "ตัวแปรอ้างตัวแปร" → **นั่นคือเส้นที่ห้ามข้าม**
 */
export type VarOp = "add" | "sub" | "mul" | "div";

/**
 * 1 ขั้นของการคำนวณตัวแปร
 * ★ `op` ไม่ระบุ = `div` — ข้อมูลที่บันทึกไว้ก่อน D70 ไม่มีช่องนี้และเป็นการหารเสมอ
 *   (ค่าปริยายนี้คือสิ่งเดียวที่กันไม่ให้ตัวเลขของลูกค้าเดิมขยับ **ห้ามเปลี่ยน**)
 */
export type VarStep = { op?: VarOp; kind: VarSource; value?: number; inputKey?: string };

/** ชื่อเดิมสมัยที่ยังหารได้อย่างเดียว — คงไว้ให้โค้ดเก่าที่อ้างถึงยังคอมไพล์ผ่าน */
export type VarDivisor = VarStep;

/**
 * ความละเอียดของค่าที่ตัวแปรเก็บ
 * 🪤 ค่าปริยาย `none` = ไม่ปัดเลย (พฤติกรรมเดิมก่อน D70) — **ห้ามเปลี่ยนค่าปริยาย**
 *    เปลี่ยนเมื่อไหร่ = อัตราต่อชั่วโมงของลูกค้าที่ตั้งไว้แล้วขยับเงียบ ๆ ทุกเจ้าพร้อมกัน
 */
export type VarRounding = "none" | "int" | "dec2";
...
export type PayVariable = {
  ...
  inputKey?: string;
  /**
   * ขั้นการคำนวณต่อจากตัวตั้ง — คิด **เรียงซ้ายไปขวาทีละขั้น ไม่มีลำดับความสำคัญ**
   * `ฐาน − A ÷ B` ที่นี่ = `((ฐาน − A) ÷ B)` **ไม่ใช่** `ฐาน − (A ÷ B)` ตามกฎคณิตศาสตร์
   * → หน้าจอต้องแสดงวงเล็บตามลำดับที่คิดจริงเสมอ (`variableFormulaText`)
   */
  steps?: VarStep[];
  /**
   * @deprecated ชื่อเดิมของ `steps` สมัยที่หารได้อย่างเดียว
   * ★ ยังอ่านอยู่โดยตั้งใจ เพื่อให้ golden test ชุดก่อน D70 ผ่านโดยไม่ต้องแก้ไฟล์เทส
   *   (= หลักฐานว่าเส้นทางคำนวณเดิมไม่ขยับ) · โค้ดใหม่ให้ใช้ `steps` เท่านั้น
   */
  divisors?: VarStep[];
  /** ปัดค่าที่ได้ — ไม่ระบุ = ไม่ปัด (พฤติกรรมเดิม) */
  rounding?: VarRounding;
  sort?: number;
  active?: boolean;
};
