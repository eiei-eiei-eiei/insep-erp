 ...
 * strip คอลัมน์ `id` (bigserial) ทุกตาราง — ไม่มี FK อ้าง id → restore ให้ DB แจกใหม่ (RESTART IDENTITY)
 */
...
/** ดึงทุกตาราง (strip id) → payload + row_counts */
async function dumpAll(admin: SupabaseClient): Promise<{ payload: Payload; counts: Record<string, number> }> {
...
export async function takeSnapshot(opts: { name: string; createdBy: string; isAuto?: boolean }) {
  const admin = createAdminClient();
  const { payload, counts } = await dumpAll(admin);
  const { data, error } = await admin
    .from("snapshots")
    .insert({ name: opts.name, created_by: opts.createdBy, is_auto: opts.isAuto ?? false, row_counts: counts, payload })
 ...
 */
async function reseedIdCounters(admin: SupabaseClient) {
  ...
  const { data: txs } = await admin.from("transactions").select("tx_id, transfer_id");
  for (const t of txs ?? []) {
  ...
  const { data: os } = await admin.from("sales_orders").select("qu_no, order_no");
  for (const o of os ?? []) {
  ...
  const { data: cs } = await admin.from("contacts").select("contact_id");
  for (const c of cs ?? []) {
    const m = strOf(c, "contact_id").match(/^C-(\d+)$/);
    if (m) bump("CONTACT", Number(m[1]));
  }
  const { data: bs } = await admin.from("bank_accounts").select("account_id");
  for (const b of bs ?? []) {
    const m = strOf(b, "account_id").match(/^ACC-(\d+)$/);
    if (m) bump("BANK_ACC", Number(m[1]));
  }

  const ups = [...counterMax.entries()].map(([key, value]) => ({ key, value }));
  if (ups.length) {
    const { error } = await admin.from("counters").upsert(ups);
    if (error) throw new Error(`reseed counters: ${error.message}`);
  }
}

/** ย้อนข้อมูลกลับไปตาม snapshot id (ล้างทั้งหมดแล้วโหลดจาก payload) */
export async function restoreSnapshot(id: number) {
  const admin = createAdminClient();
  const { data: snap, error } = await admin.from("snapshots").select("payload").eq("id", id).single();
  if (error || !snap) throw new Error(`ไม่พบ snapshot #${id}`);
  const payload = snap.payload as Payload;

  await rpc(admin, "fn_mig_truncate");
  await rpc(admin, "fn_mig_set_triggers", { p_enable: false });
  try {
    for (const t of SNAPSHOT_ORDER) {
      const rows = payload[t] ?? [];
      if (rows.length) await insertBatch(admin, t, rows);
    }
  } finally {
    await rpc(admin, "fn_mig_set_triggers", { p_enable: true });
  }
  await rpc(admin, "fn_mig_recompute_stock");
  await reseedIdCounters(admin); // กัน id ชนหลัง restore (snapshot อาจเก็บ counter เก่า)
}

/** เทียบ row_counts ของ snapshot กับสภาพปัจจุบัน (preview ผลกระทบ) */
export async function previewRestore(id: number) {
  const admin = createAdminClient();
  const { data: snap, error } = await admin.from("snapshots").select("name, created_at, row_counts").eq("id", id).single();
  if (error || !snap) throw new Error(`ไม่พบ snapshot #${id}`);
