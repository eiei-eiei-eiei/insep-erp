...
| — | **ฐาน multi-tenant — ขายเป็นสินค้าได้จริง** (D46-D48) | ✅ (2026-08-11) · **อยู่บน branch `feat/multi-tenant` ยังไม่ push · DB production ยังอยู่ที่ 0024** · migration **0025-0032** · test **241** + `npm run test:tenant` **58** (ยิง Supabase จริง) · `tenants` + `my_tenant()` + `tenant_id` 31 ตาราง (default+RLS → `.from()` 174 จุดไม่ต้องแก้) · `entity_id` 16 ตาราง + `my_default_entity()` · ผ่าตัด PK/unique **23 จุด** เป็น composite · RLS 56 policy · อุด definer 7 ตัว + service-role path (snapshot/restore เคยดูดข้ามลูกค้า) · หน้า login แสดงแบรนด์ตาม subdomain (co-brand) · แบรนด์แหล่งเดียวที่ `app_settings` (D47) · รหัสตั้งต้นสุ่มไม่ซ้ำ + บังคับเปลี่ยนครั้งแรก + **ชื่อผู้ใช้ห้ามซ้ำทั้งระบบ** (D48) · ชุดเทส `docs/TESTING.md` ส่วนที่ 26 · **เหลือ**: ผู้ใช้เทสในเบราว์เซอร์ · ขั้น 6 ย้าย DB จริง · 🚨 **MFA ก่อนรับลูกค้ารายแรก** |
